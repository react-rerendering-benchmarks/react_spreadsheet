...
const Spreadsheet = memo(function Spreadsheet() {
  ...
  const [sheet, setSheet] = useState<string[][]>(() => new Array(100).fill(new Array(100).fill("")));
  ...
  function parseCellValue(value: string): string {
    ...
    if (matches === null || operations[operations.length - 1] !== "") return "Incorrect formula!";
    let result = 0;
    matches.forEach((val, index) => {
      // TODO add proper order of operations
      const [col, row] = val.match(/([A-Z]+)(\d+)/)!.slice(1);
      const cellValue = cellValues[`${parseInt(row) - 1},${translateColumnNameToIndex(col)}`];
  ...
  const updateValue = useCallback(function updateValue(newValue: string, row: number, col: number) {
  ...
  const selectCell = useCallback(function selectCell(row: number, col: number) {
    setSelectedCell({
      row,
      col
    });
  ...
  return <div className="spreadsheet-wrapper">
      <div className="spreadsheet">
        <div className="row header">
          ...
          {sheet[0].map((col, j) => <div className="cell header" key={j}>
              {translateColumnIndexToName(j)}
            </div>)}
        </div>
        {sheet.map((row, i) => <div key={i} className="row">
            <div className="cell header" key={-1}>
              {i + 1}
            </div>
            {row.map((cell, j) => <Cell key={j} value={cellValues[`${i},${j}`] || ""} displayValue={parseCellValue(cellValues[`${i},${j}`] || "")} updateValue={updateValue} selectCell={selectCell} isSelected={selectedCell?.row === i && selectedCell?.col === j} row={i} col={j} />)}
          </div>)}
      </div>
    </div>;
});
